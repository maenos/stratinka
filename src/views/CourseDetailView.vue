...
const submitComment = async () => {
  ...
  await commentStore.postComment(route.params.slug, {
    content: newCommentContent.value,
    rating: newCommentRating.value || null,
    parentId: null,
  })
        ...
        <div class="container mx-auto px-4 grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          ...
          <div class="lg:col-span-2 space-y-4">
            <div class="badge badge-primary font-bold">
              {{ courseStore.currentCourse.category }}
            </div>
            ...
            <div class="flex flex-wrap gap-4 items-center text-sm mt-4">
              <div class="flex items-center gap-2">
                ...
                <span class="opacity-70"
                  >({{ courseStore.currentCourse.students.toLocaleString() }} avis)</span
                >
              </div>
              <div class="flex items-center gap-2">
                <svg
                ...
                <span
                  ...
                  <span class="font-bold underline cursor-pointer">{{
                    courseStore.currentCourse.author.name
                  }}</span></span
                >
              </div>
              <div class="flex items-center gap-2">
        ...
        <div class="lg:w-2/3 space-y-8">
          ...
          <div class="border border-base-200 rounded-lg p-6 bg-base-100">
            ...
            <ul class="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
              <li
                v-for="(item, index) in courseStore.currentCourse.whatYouWillLearn"
                :key="index"
                class="flex gap-2 items-start"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="h-5 w-5 text-accent shrink-0"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
          ...
          <div>
            ...
            <div
              v-for="(section, idx) in courseStore.currentCourse.sections"
              :key="idx"
              class="collapse collapse-arrow border border-base-200 bg-base-100 mb-2 rounded-box"
            >
              <input type="checkbox" :checked="idx === 0" />
              <div class="collapse-title text-base font-medium bg-base-200/50">
                {{ section.title }}
                <span class="text-xs font-normal opacity-60 ml-2 block sm:inline sm:ml-4"
                  >{{ section.lessons.length }} sessions</span
                >
              </div>
          ...
          <!-- ══════════════════════════════════════════ -->
          <div class="mt-4">
            <h2 class="text-xl font-bold mb-6">Commentaires & Avis</h2>

            <!-- Loading -->
            <div v-if="commentStore.loading" class="flex flex-col gap-4">
              <div v-for="n in 3" :key="n" class="flex gap-3 animate-pulse">
                <div class="w-10 h-10 rounded-full bg-base-300 shrink-0"></div>
            ...
            <div v-else>
              ...
              <div
                ...
                class="mb-8 p-5 border border-base-200 rounded-xl bg-base-100"
              >
                <p class="font-semibold mb-3 text-sm">Laisser un commentaire</p>
                ...
                <div class="flex items-center gap-1 mb-3">
                  ...
                  >
                    <span :class="star <= newCommentRating ? 'text-yellow-400' : 'text-base-300'"
                      >★</span
                    >
                  </button>
                  <span v-if="newCommentRating" class="text-xs text-base-content/50 ml-2"
                    >{{ newCommentRating }}/5</span
                  >
                </div>
              ...
              <div
                v-else
                class="mb-8 p-4 border border-base-200 rounded-xl bg-base-200/40 text-sm text-center"
              >
                <span class="text-base-content/60"
                  >Connectez-vous pour laisser un commentaire.</span
                >
              </div>
              ...
              <div v-else class="flex flex-col gap-6">
                <div v-for="comment in commentStore.comments" :key="comment.id" class="group">
                  <!-- Comment root -->
                  <div class="flex gap-3">
                    <img
                      :src="comment.user.avatar"
                      :alt="comment.user.name"
                      class="w-10 h-10 rounded-full object-cover shrink-0 border border-base-200"
                    />
                    <div class="flex-1 min-w-0">
                      <div class="flex flex-wrap items-center gap-2 mb-1">
                        <span class="font-semibold text-sm">{{ comment.user.name }}</span>
                        <!-- Star rating display -->
                        <span v-if="comment.rating" class="flex">
                          <span
                            v-for="s in 5"
                            :key="s"
                            :class="s <= comment.rating ? 'text-yellow-400' : 'text-base-200'"
                      ...
                      <div v-if="replyingTo === comment.id" class="mt-3 flex gap-2">
                        ...
                        <button
                          ...
                          :disabled="!replyContent.trim() || commentStore.submitting"
                        >
                          <span
                            v-if="commentStore.submitting"
                            class="loading loading-spinner loading-xs"
                      ...
                      <div
                        ...
                        class="mt-4 ml-4 pl-4 border-l-2 border-base-200 flex flex-col gap-4"
                      >
                        <div v-for="reply in comment.replies" :key="reply.id" class="flex gap-3">
                          <img
                            :src="reply.user.avatar"
                            :alt="reply.user.name"
                            class="w-8 h-8 rounded-full object-cover shrink-0 border border-base-200"
                          />
                          <div class="flex-1">
          ...
          <div class="sticky top-24">
            <div class="card bg-base-100 shadow-xl border border-base-200 overflow-hidden">
              ...
              <div class="relative aspect-video bg-black group cursor-pointer">
                <img
                  :src="courseStore.currentCourse.thumbnail"
                  class="w-full h-full object-cover opacity-80"
                />
                <div class="absolute inset-0 flex items-center justify-center">
                  <div
                    class="bg-white rounded-full p-4 shadow-lg group-hover:scale-110 transition-transform"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      class="h-8 w-8 text-black ml-1"
                      viewBox="0 0 20 20"
                      fill="currentColor"
                    >
                      <path
                ...
                <div
                  class="absolute bottom-4 left-0 right-0 text-center font-bold text-white drop-shadow-md"
                >
                  Aperçu du cours
                </div>
              </div>
              ...
              <div class="card-body p-6">
                <div class="text-3xl font-bold mb-4">
                  {{
                    courseStore.currentCourse.price === 0
                      ? 'Gratuit'
                      : courseStore.currentCourse.price + ' €'
                  }}
                </div>
                ...
                <button
                  ...
                  @click="
                    authStore.isAuthenticated
                      ? $router.push(`/course/${courseStore.currentCourse.slug}/learn`)
                      : $router.push('/login')
                  "
                >
                  {{
                ...
                <div class="text-sm space-y-2">
                  ...
                  <div class="flex gap-2 items-center">
                    ...
                    {{ courseStore.currentCourse.duration }} de vidéo à la demand
                  </div>
                  <div class="flex gap-2 items-center">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
